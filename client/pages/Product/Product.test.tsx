...
test('Product page shows banana name', async () => {
...
test('Product page shows banana image', async () => {
  vi.mocked(getProductByIdApi).mockResolvedValue(banana)
...
test('Product page shows FAQ section', async () => {
  vi.mocked(getProductByIdApi).mockResolvedValue(banana)
  ...
  const faq = await findAllByRole('heading', { level: 2 })
  expect(faq[0].textContent).toMatch('FAQ')
})
